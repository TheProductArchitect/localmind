...
export async function PUT(req: NextRequest, { params }: { params: { modelName: string } }) {
...
export async function DELETE(_req: NextRequest, { params }: { params: { modelName: string } }) {
  const name = decodeURIComponent(params.modelName);
  deleteOverride(name);
  return NextResponse.json({ ok: true });
}

export async function GET(_req: NextRequest, { params }: { params: { modelName: string } }) {
  const name = decodeURIComponent(params.modelName);
